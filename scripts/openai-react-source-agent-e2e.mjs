...
function validateAttributedRegression(evidence) {
  ...
  candidateStylesPathFromSource(finding.source);
  return finding;
}
...
const tools = [
  {
    ...
    description:
      'Run Viewportable Canonical Agent Evidence V2 against the real baseline and candidate React renderer. Call this first and after every source edit. Success requires outcome clean.',
    parameters: {
      type: 'object',
      properties: {},
      required: [],
  ...
  {
    type: 'function',
    name: 'read_attributed_css',
    description:
      'Read only the authored CSS block identified by the deterministic source field of the current Viewportable finding. A full-file source read is intentionally unavailable.',
    parameters: {
      type: 'object',
      properties: {},
      required: [],
  ...
  {
    type: 'function',
    name: 'replace_in_attributed_css',
    description:
      'Apply one exact minimal replacement inside the CSS file identified by the current Viewportable source attribution.',
    parameters: {
      type: 'object',
      properties: {
        oldText: {
...
const instructions = [
  ...
  'Use finding.source from Viewportable Canonical Agent Evidence V2 as the source of truth for what authored CSS to inspect.',
  'There is deliberately no generic full-file source read tool.',
  'Use read_attributed_css to inspect only the CSS block proven by Viewportable.',
  'You may modify only the attributed CSS through replace_in_attributed_css.',
  'Make the smallest exact source replacement that fixes the regression. Never rewrite or reformat the whole file.',
  'After every replace_in_attributed_css call, call viewportable_compare again.',
...
async function runTool(name, args) {
  ...
  if (name === 'read_attributed_css') {
    const finding = validateAttributedRegression(latestEvidence);
    const stylesPath = candidateStylesPathFromSource(finding.source);
    const content = await readFile(stylesPath, 'utf8');
    const snippet = attributedSnippet(content, finding.source);
    ...
    return { source: finding.source, content: snippet };
  }

  if (name === 'replace_in_attributed_css') {
    writes += 1;
    ...
    const attributed = attributedSnippet(current, finding.source);
    assert(
      attributed.includes(args.oldText) || attributed === args.oldText,
      'replacement oldText is outside the attributed CSS block',
    );
...
try {
  ...
  const preflightAttributedFinding = validateAttributedRegression(preflight);
  ...
  const acceptance = {
    ...
    engineCommit: '238dc2df1752e03a005c8077506ba5496de6be5c',
    model,
    schemaVersion: first.schemaVersion,
    result: 'pass',
    widths,
    expectedExactRange: { minWidth: expectedMinWidth, maxWidth: expectedMaxWidth },
    preflightSource: preflightAttributedFinding.source,
    responseIds,
    usage,
    toolCalls,
    writes,
