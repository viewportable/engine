...
22. **Scan-Mode Authored Repair V1 - complete.** Scan-mode horizontal-overflow root causes now use the same Source Location V1 and Source Map Mapping V1 pipeline as deterministic compare protrusions. V3/V4 map root-cause source evidence onto grouped scan findings via `groupId`. A real Sass/Vite production acceptance requires a scan finding to map back to `Candidate.source.scss`, permits one narrow authored-line edit, rebuilds, and verifies a clean `viewportable_scan` result.
