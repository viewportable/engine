...
5. **Intentional-reflow classification policy - current.** Use the new evidence to distinguish expected responsive flow from suspicious isolated wrapping without hiding real failures. Do not treat `flex-wrap: wrap` alone as sufficient: the reviewed Duolingo negative range also contains repeated non-flex inline-flow wrapping. Avoid corpus-specific transition-count thresholds.
6. **Relationship intervals.** Generalize only the relationships required by later small-range analysis instead of building a full graph up front.
7. **Small-range anomaly research.** Evaluate after wrapping precision because the corpus contains many raw Small-Range false-positive reports.
8. **Element protrusion and generic collision research.** Revisit with stronger observability/relationship evidence; the corpus shows substantial NOI pressure for these geometry-only families.
9. Keep screenshot/pixel verification optional until structural evidence shows where it reduces false positives enough to justify its cost.
