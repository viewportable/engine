...
9. **Structural base-vs-candidate diff - V1 in progress.** Match nodes conservatively across independent renders using stable authored identity first and structural-path fallback second. Compare selected relationships only when both subjects and their parent match. V1 reports relationship state changes for sibling overlap and parent containment; persistent single-version overlap/protrusion stays silent. Reparenting, appearance/disappearance, range aggregation, and CLI compare orchestration remain follow-up slices.
