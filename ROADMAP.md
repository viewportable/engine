...
15. **Deterministic Source Attribution V1 + Agent Evidence V2 - complete.** MCP now publishes `viewportable.agent-evidence.v2`. Every finding carries strict `source` semantics: uniquely proven direct CSS constraints expose stylesheet/source path, selector, property, value, and active media context; ambiguous or indirect causes remain `null`. V1 remains available for compatibility rather than being silently extended.
16. Keep screenshot/pixel verification optional until structural evidence shows where it reduces false positives enough to justify its cost.
