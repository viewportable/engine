...
19. **Real Build-Tool Source Map Acceptance V1 - complete.** CI builds a React fixture with pinned Vite, Dart Sass, CSS Modules, and a PostCSS transform, serves the production output, reproduces an exact responsive protrusion through the production CLI, proves generated CSS coordinates map back to the checked-in `Card.module.scss`, and verifies the resulting GitHub annotation path through unique suffix plus SHA-256 checkout matching. Ambiguous checkout matches fail closed.
