...
4. **Wrapping precision and canonical grouping - current.** Preserve the deterministic sibling-wrapping observation, add evidence for intentional authored/repeated reflow, and group related detector observations into canonical user-facing findings. Text-line geometry is not required for AirBnb RLF #26 and should only be introduced when an independently demonstrated text-wrapping failure requires it.
