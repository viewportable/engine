...
9. **Structural base-vs-candidate diff - current.** Compare normalized identities and selected structural relationships between two renders so the Engine can report newly introduced responsive behavior instead of guessing intent from one version in isolation. Keep generic all-pairs collision deferred.
