...
7. **Small-range anomaly research - current.** The first research-only analyzer tracks stable sibling pairs and emits sampled `separate -> overlap -> separate` candidates without producing CLI issues. Next, evaluate those candidates against the Small-Range oracle and anti-oracle before adding exact boundary refinement or any default detector behavior.
