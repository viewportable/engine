...
17. **GitHub Source Annotations V1 - complete.** Deterministic introduced CSS findings can be attached directly to the matching PR source line and columns through the existing managed Check Run. Standalone Action mode requires a candidate `source-root`, maps only stylesheets inside that checkout, and re-verifies the exact property/value range against the local file before publishing. Cloud/App mode accepts only normalized repo-relative annotations from the authenticated executor. Transformed or unverifiable stylesheet coordinates remain unannotated until Source Map Mapping V1.
18. Keep screenshot/pixel verification optional until structural evidence shows where it reduces false positives enough to justify its cost.
