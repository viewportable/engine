...
7. **Small-range anomaly research - current.** Build one detector-specific experiment on top of sampled intervals, then add boundary refinement only where the experiment proves it is needed. The corpus contains many raw Small-Range false-positive reports, so candidate generation and verdict policy must remain separate.
