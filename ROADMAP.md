...
14. **Canonical Agent Evidence Contract V1 - complete.** `viewportable.agent-evidence.v1` is now the strict semantic boundary between rich Engine reports and agent surfaces. MCP uses the same schema as `outputSchema`; runtime parsing rejects unknown fields; scan and compare evidence normalize into one finding language with explicit versioning and a pointer to the full artifact.
15. Keep screenshot/pixel verification optional until structural evidence shows where it reduces false positives enough to justify its cost.
